...
const POST_AUTH_REDIRECT_STORAGE_KEY = "auth_v2.redirect";
...
export const buildAuthRoute = ({
  ...
  if (safeRedirectTarget) {
    params.set(AUTH_V2_QUERY_PARAMS.REDIRECT, safeRedirectTarget);
  }

  const queryString = params.toString();

  if (!queryString) {
    return AUTH_V2_ROUTES.ROOT;
  }

  return `${AUTH_V2_ROUTES.ROOT}?${queryString}`;
};

export const getRedirectFromSearchParams = (search: string) => {
  const params = new URLSearchParams(search);
  return getSafeRedirectTarget(params.get(AUTH_V2_QUERY_PARAMS.REDIRECT));
};
