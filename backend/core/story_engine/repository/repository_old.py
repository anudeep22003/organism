...
class RepositoryOld:
    ...
    async def get_all_projects_of_user_with_story_count(
        ...
        return await self.repository_v2.get_all_projects_of_user_with_story_count(
            user_id
        )

    async def create_project(self, user_id: str, name: str | None = None) -> Project:
        project = await self.repository_v2.create_project(user_id=user_id, name=name)
    ...
    async def get_project_details(
        ...
        return await self.repository_v2.get_project_details(
            user_id=user_id,
            project_id=project_id,
        )

    async def create_new_story(self, project_id: uuid.UUID) -> Story:
        story = await self.repository_v2.create_new_story(project_id)
        await self.db.commit()
    ...
    async def get_story(
        ...
        return await self.repository_v2.get_story(project_id, story_id)

    async def delete_story(self, project_id: uuid.UUID, story_id: uuid.UUID) -> None:
        await self.repository_v2.delete_story(project_id, story_id)
        await self.db.commit()

    async def get_story_with_project(self, story_id: uuid.UUID) -> Story | None:
        return await self.repository_v2.get_story_with_project(story_id)
    ...
    async def update_story_with_story_text_and_user_input_text(
        ...
        story = (
            await self.repository_v2.update_story_with_story_text_and_user_input_text(
                story_id=story_id,
    ...
    async def create_edit_event(
        ...
        event = await self.repository_v2.create_edit_event(
            project_id=project_id,
    ...
    async def complete_edit_event(
        ...
        event = await self.repository_v2.update_edit_event(
    ...
    async def get_edit_events_for_target(
        ...
        return await self.repository_v2.get_edit_events_for_target(
            target_type=target_type,
            target_id=target_id,
            limit=limit,
        )
    ...
    async def bulk_create_characters(self, characters: list[Character]) -> None:
        await self.repository_v2.bulk_create_characters(characters)
        await self.db.commit()

    async def get_all_characters_for_a_story(
        self, story_id: uuid.UUID
    ) -> list[Character]:
        return await self.repository_v2.get_all_characters_for_a_story(story_id)

    async def get_character(
        self, character_id: uuid.UUID, story_id: uuid.UUID
    ) -> Character | None:
        return await self.repository_v2.get_character(character_id, story_id)
    ...
    async def get_character_for_user_in_project_and_story(
        ...
        return await self.repository_v2.get_character_for_user_in_project_and_story(
            user_id=user_id,
            project_id=project_id,
            story_id=story_id,
    ...
    async def update_character(
        ...
        character = await self.repository_v2.update_character(
            character_id=character_id,
            story_id=story_id,
            updates=updates,
        )
    ...
    async def replace_character_attributes(
        ...
        character = await self.repository_v2.replace_character_attributes(
            character_id=character_id,
            story_id=story_id,
    ...
    async def delete_character(
        ...
        await self.repository_v2.delete_character(character_id, story_id)
        await self.db.commit()

    async def update_character_render_url(
        self, character_id: uuid.UUID, story_id: uuid.UUID, render_url: str
    ) -> Character:
        character = await self.repository_v2.update_character_render_url(
            character_id=character_id,
            story_id=story_id,
